...
function WeightedSocket(reactivesocket, opts) {
    ...
    reactivesocket._stamp0 = reactivesocket._stamp;   // last ts we sent a req. or receive a resp.
    ...
    var _instantaneous = function _instantaneous(now) {
        return reactivesocket._duration + (now - reactivesocket._stamp0) * reactivesocket._outstandings;
    };

    var _incr = function _incr() {
        var now = CLOCK.now();
        reactivesocket._duration += (now - reactivesocket._stamp0) * reactivesocket._outstandings;
        reactivesocket._outstandings += 1;
        reactivesocket._stamp = now;
        reactivesocket._stamp0 = now;
        console.log('incr duration = ' + reactivesocket._duration + ' outstanding = ' + reactivesocket._outstandings);
        return now;
    };
    ...
    var _decr = function _decr(ts) {
        ...
        reactivesocket._outstandings -= 1;
        reactivesocket._stamp0 = now;
        console.log('decr duration = ' + reactivesocket._duration + ' outstanding = ' + reactivesocket._outstandings);
        return now;
    };
    ...
    reactivesocket.getPredictedLatency = function getPredictedLatency() {
        ...
        } else if (reactivesocket._outstandings === 0 && elapsed > reactivesocket._inactivityPeriodMs) {
            // if we did't see any data for a while, we decay the prediction by
            // inserting artificial low value into the median
            reactivesocket._median.insert(reactivesocket._median.estimate() * 0.8);
            reactivesocket._stamp = now;
            reactivesocket._stamp0 = now;
