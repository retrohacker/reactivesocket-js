...
function WeightedSocket(reactivesocket, opts) {
    ...
    this._inactivityPeriodMs = opts.inactivityPeriodMs;
    this._median = new SlidingMedian(opts.medianBufferSize);
}
...
WeightedSocket.prototype.getPredictedLatency = function getPredictedLatency() {
    ...
    } else if (self._outstandings === 0 && elapsed > self._inactivityPeriodMs) {
        // if we did't see any data for a while, we decay the prediction by
        // inserting artificial low value into the median
        self._median.insert(self._median.estimate() * 0.8);
        console.log(Date.now() + ' INACTIVITY PERIOD!!!!');
        self._stamp = now;
        self._stamp0 = now;
