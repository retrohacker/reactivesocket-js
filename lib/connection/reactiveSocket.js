...
function ReactiveSocket(opts) {
    ...
    var self = this;
    this._recorder = (opts.recorder || DISABLE_RECORDER).scope('rs');
    this._metrics = {
        ...
        setupError: self._recorder.counter('setupError'),
        parseError: self._recorder.counter('parseError'),
        transportError: self._recorder.counter('transportError'),
        serializeError: self._recorder.counter('serializeError'),
        framingError: self._recorder.counter('framingError'),
        timeoutCounter: self._recorder.counter('timeouts'),

        setupLatency: self._recorder.timer('setupLatency'),
        requestLatency: self._recorder.timer('requestLatency')
    };
    ...
    self._log.trace({opts: opts}, 'rs.connection: new');

    this._type = opts.type;
    this._isSetup = false;
    this._version = (self._type === 'client') ? CONSTANTS.VERSION : null;
    this._keepalive = opts.keepalive || 5 * 1000;
    ...
    self._transportStream.once('close', function onClose() {
        ...
        _(self._streams.streams).forEach(function (stream) {
            ...
            self._log.debug({streamId: streamId},
...
util.inherits(ReactiveSocket, EventEmitter);
 ...
 * received
 */
ReactiveSocket.prototype.request = function request(req) {
    var self = this;
    ...
    stream.on('timeout', function onTimeout () {
        self._log.debug({stream: stream, request: req}, 'Timeouting request');
 ...
 */
ReactiveSocket.prototype.setup = function setup(su, cb) {
    var self = this;
 ...
 * @returns {null}
 */
ReactiveSocket.prototype.send = function send(frame, cb) {
    var self = this;
    self._log.trace({frame: frame}, 'Connection.send: entering');
 ...
 * @returns {null}
 */
ReactiveSocket.prototype.sendLease = function send(numberOfRequest, ttl) {
    var self = this;
    self._log.trace({numberOfRequest: numberOfRequest, ttl: ttl},
 ...
 * @param {Function} cb Callback to call after close
 *
 * @returns {null}
 */
ReactiveSocket.prototype.close = function close(cb) {
    var self = this;
...
ReactiveSocket.prototype._handleSetup = function _handleSetup(frame) {
    var self = this;
    self._log.trace({frame: frame}, 'Connection._handleSetup: entering');
    ...
    self._log.trace({setup: self._isSetup}, 'Connection._handleSetup: exiting');
};

ReactiveSocket.prototype._handleLease = function _handleLease(frame) {
    var self = this;
    self._log.trace({frame: frame}, 'Connection._handleLease: entering');

    self._leaseNumberOfRequests = frame.budget;
    self._leaseExpirationDate = Date.now() + frame.ttl;

    self.emit('lease', frame);
    self._log.trace({setup: self._isSetup}, 'Connection._handleLease: exiting');
};

ReactiveSocket.prototype._handleKeepalive = function _handleKeepalive(frame) {
    var self = this;
    self._log.trace({frame: frame}, 'Connection._handleKeepalive: entering');
...
ReactiveSocket.prototype._handleRequest = function _handleRequest(frame) {
    var self = this;
...
ReactiveSocket.prototype._handleResponse = function _handleResponse(frame) {
    var self = this;
...
ReactiveSocket.prototype._handleError = function _handleError(frame) {
    var self = this;
    self._log.trace({frame: frame}, 'Connection._error: entering');
...
ReactiveSocket.prototype._handleCancel = function _handleCancel(frame) {
    var self = this;
    self._log.trace({frame: frame}, 'Connection._cancel: entering');
...
ReactiveSocket.prototype._sendKeepalive =
    function _sendKeepalive(requireResponse) {
        var self = this;
        self._log.trace({requireResponse: requireResponse},
...
ReactiveSocket.prototype._getNewStream = function _getNewStream() {
    var self = this;
    self._log.trace({latest_id: self._streams.latest},
    ...
    self._log.trace({latest_id: self._streams.latest, id: id},
                    'Connection._getNewStream: exiting');
    return stream;
};

// Responding streams will invoke this to persist a stream id
ReactiveSocket.prototype._getStream = function _getStream(id) {
    var self = this;
...
ReactiveSocket.prototype._deleteStream = function _deleteStream(id) {
    var self = this;
