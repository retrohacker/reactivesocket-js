...
function TcpConnectionPool(opts) {
    ...
    function connect(cb) {
        cb = _.once(cb);
        //TODO: what if there are no more free hosts
        //var freeList = _.keys(self._connection.free);
        //if (freeList.length === 0) {
        //self.warn('TcpConnectionPool.connect: no more free connections');
        //return cb();
        //}
 ...
 * @returns {null}
 */
TcpConnectionPool.prototype.updateHosts = function updateHosts(hosts) {
    var self = this;
    self._log.info({hosts: hosts}, 'TcpConnectionPool.updateHosts: entering');
    ...
    var deadHosts = _.difference(self._hosts, hosts);
