...
function Connection(opts) {
    ...
    this._close = self._recorder.counter('close');
    this._requestCounter = self._recorder.counter('requests');
    this._responseCounter = self._recorder.counter('responses');
    this._errorCounter = self._recorder.counter('errors');
    this._setupError = self._recorder.counter('setup_error');
    this._parseError = self._recorder.counter('parse_error');
    this._transportError = self._recorder.counter('transport_error');
    this._serializeError = self._recorder.counter('serialize_error');
    this._framingError = self._recorder.counter('framing_error');
    this._timeoutCounter = self._recorder.counter('timeouts');

    this._setupLatency = self._recorder.timer('setup_latency_ms');
    this._requestLatency = self._recorder.timer('request_latency_ms');
    ...
    self._pStream.on('data', function onRead(frame) {
        ...
        switch (frame.header.type) {
            case TYPES.ERROR:
                self._errorCounter.incr();
                self._handleError(frame);
                break;
            case TYPES.RESPONSE:
                self._responseCounter.incr();
    ...
    if (self._type === 'server') {
        ...
        var setupLatencyTimerId = self._setupLatency.start();
        self.setup({
            lease: opts.lease,
                ...
                if (err) {
                    ...
                    if (opts.lease) {
                        self.once('lease', function onFirstLease(frame) {
                            ...
                            self._setupLatency.stop(setupLatencyTimerId);
                            self.emit('ready');
                        });
                    } else {
                        self._setupLatency.stop(setupLatencyTimerId);
