 ...
 */
function SlidingMedian(_opts) {
    var opts = _opts || {};
    assert.optionalNumber(opts.size, 'opts.size');
    assert.optionalObject(opts.clock, 'opts.clock');
    assert.optionalNumber(opts.windowMs, 'opts.windowMs');

    var self = this;
    this._size = opts.size || 64;
    this._clock = opts.clock || CLOCK;
    this._windowMs = opts.windowMs || 60 * 1000;
    this._deltaT = new Ewma(self._windowMs, 1, self._clock);
    this._multiplicity = 1;

    this._stamp = self._clock.now();
...
SlidingMedian.prototype.insert = function insert(x) {
    ...
    self._stamp = now;
    self._deltaT.insert(elapsed);

    // compute how many data point to insert in the buffer based on the
    // estimated RPS and the considered window duration.
    var deltaT = self._deltaT.value();
    var rpms = 1 / deltaT;

    self._multiplicity += (self._size * deltaT) / self._windowMs;
    var multiplicity = self._multiplicity;
    var savedMultiplicity = multiplicity;

    if (multiplicity > 1) {
        self._multiplicity -= multiplicity >> 0;
    }
    ...
    //     + '\trps: ' + (1000 * rpms)
    //     + '\tdeltaT: ' + deltaT
    //     + '\tmultiplicity: ' + savedMultiplicity
    //     + ',\trounded to ' + multiplicity
    //     + ',\tvalue: ' + x);
    ...
    // var rest = multiplicity - (multiplicity >> 0);
    // multiplicity = multiplicity >> 0;

    // if (Math.random() < rest) {
    //     multiplicity += 1;
    // }
    // multiplicity = Math.min(self._size, multiplicity);
...
SlidingMedian.prototype.insertOne = function insertOne(x) {
    var self = this;
